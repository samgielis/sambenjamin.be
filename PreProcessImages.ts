...
function preProcessStories() {
  ...
  for (
    let fileIndex = 0;
    fileIndex < filesInStoriesDirectory.length;
    fileIndex++
  ) {
    const folderURI = path.join(
      storiesFolderPath,
      filesInStoriesDirectory[fileIndex]
    );
...
function augmentStoryWithExifData(story: Story, storyFolderURI: string): Story {
  for (let i = 0; i < story.photos.length; i++) {
    for (let j = 0; j < story.photos[i].length; j++) {
      const photo = story.photos[i][j];
      const exifData = readExifData(path.join(storyFolderURI, photo.fileName));
      story.photos[i][j] = { ...photo, ...exifData };
    }
  }
  return story;
}
