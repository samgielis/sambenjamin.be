...
export function StoryCover(props: StoryCoverProps) {
  const titles = [];
  titles.push(<h1 key="title">{props.story.title}</h1>);

  if (props.story.subTitle) {
    titles.push(<h2 key="subtitle">{props.story.subTitle}</h2>);
  }
  ...
  if (props.story.instaHandle) {
    titles.push(
      <Button
        ...
        href={`https://instagram.com/${props.story.instaHandle}`}
      >
    ...
    <div className="story-cover">
      <Container className="d-flex flex-column align-items-center ">
        <div
          ...
          style={{
            backgroundImage: `url("${getStoryCoverImageURL(props.story)}")`,
          }}
        ></div>
